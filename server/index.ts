...
// Create a live preview showing real tire inventory for client demonstration
app.get("/live-preview", async (req, res) => {
  try {
    // Fetch live products from Cin7
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>🔴 LIVE: Reivilo B2B Portal - Client View</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Segoe UI', system-ui, sans-serif; background: #f8fafc; color: #334155; }
        .header { background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); color: white; padding: 2rem 0; text-align: center; }
        .header h1 { font-size: 2.5rem; font-weight: 700; margin-bottom: 0.5rem; }
        .header .subtitle { font-size: 1.1rem; opacity: 0.9; }
        .live-indicator { background: #ef4444; color: white; padding: 0.5rem 1rem; border-radius: 20px; font-weight: 600; display: inline-block; margin: 1rem 0; animation: pulse 2s infinite; }
        @keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.7; } }
        .container { max-width: 1400px; margin: 0 auto; padding: 2rem; }
        .warehouses { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1rem; margin-bottom: 2rem; }
        .warehouse { background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; }
        .warehouse h3 { color: #1e3a8a; font-size: 1.2rem; margin-bottom: 0.5rem; }
        .products { display: grid; grid-template-columns: repeat(auto-fit, minmax(400px, 1fr)); gap: 1.5rem; }
        .product { background: white; border-radius: 12px; padding: 1.5rem; box-shadow: 0 4px 6px rgba(0,0,0,0.05); border: 1px solid #e2e8f0; transition: transform 0.2s, box-shadow 0.2s; }
        .product:hover { transform: translateY(-2px); box-shadow: 0 8px 20px rgba(0,0,0,0.1); }
        .product-header { display: flex; justify-content: between; align-items: flex-start; margin-bottom: 1rem; }
        .product-info h3 { color: #1e3a8a; font-size: 1.3rem; font-weight: 600; margin-bottom: 0.25rem; }
        .product-sku { background: #f1f5f9; color: #475569; padding: 0.25rem 0.75rem; border-radius: 6px; font-size: 0.85rem; font-weight: 500; }
        .product-category { color: #6b7280; font-size: 0.9rem; margin: 0.5rem 0; }
        .stock-info { display: grid; grid-template-columns: repeat(auto-fit, minmax(120px, 1fr)); gap: 1rem; margin: 1rem 0; }
        .stock-item { text-align: center; padding: 0.75rem; border-radius: 8px; }
        .available { background: #dcfce7; color: #166534; }
        .onhand { background: #dbeafe; color: #1d4ed8; }
        .onorder { background: #fef3c7; color: #92400e; }
        .stock-number { font-size: 1.5rem; font-weight: 700; }
        .stock-label { font-size: 0.8rem; font-weight: 500; margin-top: 0.25rem; }
        .price { color: #059669; font-size: 1.4rem; font-weight: 700; text-align: right; }
        .warehouse-breakdown { margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #e2e8f0; }
        .warehouse-breakdown h4 { color: #374151; font-size: 0.9rem; margin-bottom: 0.5rem; }
        .warehouse-stock { display: flex; justify-content: space-between; font-size: 0.85rem; margin-bottom: 0.25rem; }
        .add-to-cart { background: #1e3a8a; color: white; padding: 0.75rem 1.5rem; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; transition: background 0.2s; width: 100%; margin-top: 1rem; }
        .add-to-cart:hover { background: #1e40af; }
        .stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin-bottom: 2rem; }
        .stat { background: white; padding: 1.5rem; border-radius: 12px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05); }
...
<body>
    <div class="header">
        <h1>🏆 Reivilo B2B Portal</h1>
        <p class="subtitle">45 Years of Family Business Values Since 1980</p>
        <div class="live-indicator">🔴 LIVE DATA FROM CIN7</div>
    </div>
    
    <div class="container">
        <div class="stats">
            <div class="stat">
                <div class="stat-number">${productsData.products?.length || 0}</div>
                <div class="stat-label">Products Displayed</div>
            </div>
            <div class="stat">
                <div class="stat-number">${warehousesData?.length || 0}</div>
                <div class="stat-label">Warehouse Locations</div>
            </div>
            <div class="stat">
                <div class="stat-number">${productsData.total || 0}</div>
                <div class="stat-label">Total Available from Cin7</div>
            </div>
        </div>

        <h2 style="color: #1e3a8a; margin-bottom: 1rem;">🏪 Your Warehouse Locations</h2>
        <div class="warehouses">
            ${warehousesData?.map(warehouse => `
                <div class="warehouse">
                    <h3>${warehouse.name}</h3>
                    <p><strong>${warehouse.location}</strong></p>
                    <p style="color: #6b7280; font-size: 0.9rem;">${warehouse.description}</p>
                </div>
            `).join('') || '<p>No warehouses found</p>'}
        </div>

        <h2 style="color: #1e3a8a; margin-bottom: 1rem;">🛞 Live Tire Inventory (Real-Time from Cin7)</h2>
        <div class="products">
                ...
                <div class="product">
                    ...
                    <div class="stock-info">
                        <div class="stock-item available">
                            <div class="stock-number">${product.available}</div>
                            <div class="stock-label">AVAILABLE</div>
                        </div>
                        <div class="stock-item onhand">
                            <div class="stock-number">${product.onHand}</div>
                            <div class="stock-label">ON HAND</div>
                        </div>
                        <div class="stock-item onorder">
                            <div class="stock-number">${product.onOrder}</div>
                            <div class="stock-label">ON ORDER</div>
                        </div>
                    </div>
                    ...
                    <div class="warehouse-breakdown">
                        ...
                        <div class="warehouse-stock">
                            <span>JHB Warehouse:</span>
                            <span>${product.warehouseBreakdown?.jhb?.available || 0} available</span>
                        </div>
                        <div class="warehouse-stock">
                            <span>CPT Warehouse:</span>
                            <span>${product.warehouseBreakdown?.cpt?.available || 0} available</span>
                        </div>
                        <div class="warehouse-stock">
                            <span>BFN Warehouse:</span>
                            <span>${product.warehouseBreakdown?.bfn?.available || 0} available</span>
                        </div>
                    </div>

                    <div class="price">R${product.price} ${product.currency}</div>
                    <button class="add-to-cart" onclick="alert('Added ${product.name} to cart!')">
                        🛒 Add to Cart
                    </button>
                </div>
        ...
        <div style="background: white; padding: 2rem; border-radius: 12px; margin-top: 2rem; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            <h3 style="color: #1e3a8a; margin-bottom: 1rem;">✅ This is What Your Clients See</h3>
            <p style="color: #6b7280;">Live inventory data from your Cin7 system • Real-time stock levels • Professional B2B interface • Multi-warehouse visibility</p>
        </div>
    </div>
    ...
    <script>
        ...
        console.log('🔴 LIVE: Displaying real Cin7 inventory data');
        console.log('Products loaded:', ${productsData.products?.length || 0});
        console.log('Total Cin7 inventory:', ${productsData.total || 0});
    </script>
</body>
</html>
    `);
  } catch (error) {
    res.status(500).send(`<h1>Error loading live preview</h1><p>${error.message}</p>`);
  }
});
